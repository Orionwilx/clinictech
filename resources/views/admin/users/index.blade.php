<x-app-layout>
    <x-slot name="header">
        <x-page-header title="Usuarios" :breadcrumbs="[['label' => 'Usuarios']]">
            <x-slot:actions>
                <x-primary-link :href="route('admin.users.create')">{{ __('Nuevo usuario') }}</x-primary-link>
            </x-slot:actions>
        </x-page-header>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <x-data-table
                :cols="['w-[28%]', 'w-[28%]', 'w-[20%]', 'w-32', 'w-32']"
                :heads="[['Nombre'], ['Email'], ['Rol'], ['Estado'], ['Acciones', 'right']]">
                @forelse ($users as $user)
                    <tr class="{{ $user->trashed() ? 'bg-red-50' : 'bg-white' }}">
                        <x-td :title="$user->name">{{ $user->name }}</x-td>
                        <x-td :title="$user->email" muted>{{ $user->email }}</x-td>
                        <x-td muted>{{ $user->roles->pluck('name')->implode(', ') ?: '—' }}</x-td>
                        <x-td plain>
                            @if ($user->trashed())
                ...
                @empty
                    <tr><td colspan="5" class="px-6 py-4 text-center text-sm text-gray-500">No hay usuarios.</td></tr>
                @endforelse
            </x-data-table>
