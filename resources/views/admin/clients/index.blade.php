<x-app-layout>
    <x-slot name="header">
        <x-page-header title="Clientes" :breadcrumbs="[['label' => 'Clientes']]">
            <x-slot:actions>
                <x-primary-link :href="route('admin.clients.create')">{{ __('Nuevo cliente') }}</x-primary-link>
            </x-slot:actions>
        </x-page-header>
    </x-slot>
            ...
            <form method="GET" class="mb-4 bg-white shadow-sm sm:rounded-lg p-4">
                <div class="flex flex-wrap gap-3">
                    <div class="relative flex-1 min-w-40">
                        ...
                        <input type="text" name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Nombre, NIT o ciudad…"
                    ...
                    @if (array_filter($filters))
                        <a href="{{ route('admin.clients.index') }}" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-900">Limpiar</a>
                    @endif
                </div>
            </form>

            <x-data-table
                :cols="['w-[26%]', 'w-32', 'w-[26%]', 'w-40', 'w-28', 'w-32']"
                :heads="[['Empresa'], ['NIT'], ['Correo'], ['Ciudad'], ['Estado'], ['Acciones', 'right']]">
                @forelse ($clients as $client)
                    <tr class="{{ $client->trashed() ? 'bg-red-50' : 'bg-white' }}">
                        <x-td :title="$client->name">{{ $client->name }}</x-td>
                        <x-td muted>{{ $client->nit }}</x-td>
                        <x-td :title="$client->email" muted>{{ $client->email }}</x-td>
                        <x-td muted>{{ $client->city ?: '—' }}</x-td>
                        <x-td plain>
                            @if ($client->trashed())
                        ...
                        <x-td-actions>
                            ...
                            @else
                                <x-icon-btn :href="route('admin.clients.show', $client)" color="gray" label="Ver">
                                ...
                                <x-icon-btn :href="route('admin.clients.edit', $client)" color="brand" label="Editar">
                ...
                @empty
                    <tr><td colspan="6" class="px-6 py-4 text-center text-sm text-gray-500">No hay clientes.</td></tr>
                @endforelse
            </x-data-table>
